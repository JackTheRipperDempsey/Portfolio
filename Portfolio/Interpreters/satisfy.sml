...
| satisfy (And(p1,p2), asn, k) =
    ...
    (case satisfy(p1, asn,k) of
        true => satisfy(p2, asn, fn asn2 => (satisfy(p1,asn2,k)))
        | false => false)

| satisfy (Or(p1, p2), asn, k) =
    (case satisfy(p1, asn,k) of
        true => k asn
        | false => satisfy(p2, asn,k))
...
and falsify (Var v, asn, k) =
  ...
  | falsify (And(p1,p2), asn, k) =
     (case falsify(p1, asn,k) of
        false => falsify(p2, asn,k)
        | true => k asn)

  | falsify (Or(p1, p2), asn, k) =
     (case falsify(p1, asn,k) of
        false => false
        | true => falsify(p2, asn, fn asn2 => (falsify(p1,asn2,k))))
...
val test4 = Or(P,Not(P))
val test5 = If(P,P)
val test6 = Not(Or(P,Not(P)))
val test7 = Or(P,P)
val test8 = And(Not(P),P)
val test9 = Or(P,Q)
