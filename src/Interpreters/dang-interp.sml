...
fun push (stack, el) = (stack := el::(!stack))

fun pop stack = case (!stack) of (el::rest) => (stack := rest; el)
                | _ => raise Generic "Cannot pop an empty stack"
...
fun run (e:exp) = 
    let
        ...
        fun interp (e:exp) (env:env) = 
            ...
            |   Milestone (e1) => K.callcc( fn k => (push (miles,k); interp e1 env))

            |   Devil (e1) =>   let val v1 = interp (e1) (env) in

                                        (case (!miles) of [] =>
                                            K.callcc( fn k => (push (devils,k);v1))



                                        | (mile1::mile2) =>  K.callcc( fn k1 => (push (devils,k1);
                                                            (let val k2 = pop miles  in
                                                                (K.throw k2 v1)
                                                            end)))
            ...
            |   Angel (e1) =>   let val v1 = interp (e1) (env) in

                                        (case (!devils) of [] => v1

                                         | (devil1::devil2) => (let val k = pop devils  in
                                                                 (K.throw k v1)
                                                              end)
                                        )
                                end
